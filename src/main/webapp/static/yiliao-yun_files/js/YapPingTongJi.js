...
function showOne(index,size){
	$.getJSON("/cYaoPingTongJiController/showOne.do",{
		"index":index,
		"size":size,
		"name":$("#oneName").val(),
		"type":$("#oneType").val(),
		...
		"endDate":$("#oneEnd").val()},function(data){
			...
			$.each(data.list,function(i,v){	
					...
					h+="<td class='ng-binding'>"+v.zongCount+"</td>";
					h+="<td class='ng-binding' aria-hidden='false'>"+v.zongSum+"</td>";
				//	 c.id,c.name,c.type,SUM(quantity) zongCount,SUM(quantity*d.sale_price) zongSum
...
//					h+="<td class='ng-binding' aria-hidden='false'>"+(v.sale_price*v.counts-v.trade_price*v.counts)+"</td></tr>";
					xiaoshousum+=(v.sale_price-0)*(v.counts-0);
					pifasum+=(v.trade_price-0)*(v.counts-0);
...
function tiaoMain(gid){
	window.location.href="/cYaoPingTongJiController/showGoodsOrderMain.do?gid="+gid;
}
