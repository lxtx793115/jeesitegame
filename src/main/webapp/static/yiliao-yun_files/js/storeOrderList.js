...
function show(index,size){
	...
	$.getJSON("/cStoreOrderController/storeOrderListdata.do",{"page":index,"size":size,"no":no,"startdate":startdate,"enddate":enddate,"status":status},function(data){
		$("#shouyaoList").html('');
...
function detail(id,no,create_name,create_date,price){
	  ...
	  location.href=Window.ctx+"/../cStoreOrderController/storeReviewListDetail.do?orderId="+id+"&no="+no+"&create_name="+create_name+"&create_date="+create_date+"&price="+price;
}
