...
function show(){
	$.getJSON("/cGoodsLcController/findGoodsByqzp.do",{"name":$("#yaopinName").val(),"types":$("#yaopinType").val()},function(data){
		$("#yaopinList").html('');
	...
	//	initPage(pageCount,index);//加载页码
		$.each(data.list,function(i,v){	
				...
				h+="<li ><input value="+v.id+" style='width:16px;height:16px; vertical-align:middle; margin:auto;padding:0;' type='checkbox' onclick='dian()' name='yaopingbox'></li>";
				...
				if(v.type=="4"){
				h+="<li class='ng-binding'>器械</li>";
				}else if(v.type=="6"){
					h+="<li class='ng-binding'>营养品</li>";
				}else if(v.type=="7"){
					h+="<li class='ng-binding'>滋补品</li>";
				}
