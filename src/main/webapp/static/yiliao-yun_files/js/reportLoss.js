...
$(function(){
	...
	function order(type){
		...
		$.ajax({
      		type: "post",
       		url: "/cStoreOrderController/reportLossDo.do",
			data: {
	...
	$.getJSON("/cGoodsLcController/findBaseCGoods.do",function(data){
		...
		$.each(data,function(i,v){
			if(v.type=="2"){
				...
				j+="<td style='text-align:c' class='ng-binding'>"+v.name+"</td>";
...
function okGoods(){
	...
	$.getJSON("/cGoodsLcController/getCGoodsById.do",{"allId":h},function(data){
		...
		$.each(data,function(i,v){
			...
			//数量
			c+="<td><span><input id='quantity' type='number' style='width:50px' class='ng-pristine ng-untouched ng-valid ng-isolate-scope ng-not-empty' aria-invalid='false' onkeyup='sizeChange(this)' value='1'><span style='display:none;color:red;'>库存不足</span></span></td>";
			//批发价
			//c+="<td><span><input id='trade_price' type='text' style='width:50px' class='ng-pristine ng-untouched ng-valid ng-isolate-scope ng-not-empty' aria-invalid='false' onchange='priceChange(this)' value=''></span></td>";
			//售价
			//c+="<td><span><input hidden='true'  type='text' style='width:50px' class='ng-pristine ng-untouched ng-valid ng-isolate-scope ng-not-empty' aria-invalid='false' onchange='' value=''></span></td>";
			//有效期
			//c+="<td><span><input hidden='true' type='date' style='width:120px' class='ng-pristine ng-untouched ng-valid ng-isolate-scope ng-not-empty' aria-invalid='false' onchange='' value=''></span></td>";
			//生产日期
			//c+="<td><span><input hidden='true' type='date' style='width:120px' class='ng-pristine ng-untouched ng-valid ng-isolate-scope ng-not-empty' aria-invalid='false' onchange='' value=''></span></td>";
			//供应商
			//c+="<td><span><input hidden='true' type='text' style='width:50px' class='ng-pristine ng-untouched ng-valid ng-isolate-scope ng-not-empty' aria-invalid='false' onchange='' value='无'></span></td>";
...
function sizeChange(_this){
	...
	var size=$(_this).val()-0;//当前数量
	var gid=$(_this).parent().parent().prev().find("[type=hidden]").val();
	$.getJSON("/cGoodsStoreLcController/judgeGoodsStore.do",{"gid":gid,"sumSize":size},function(date){
		if(date==0){
...
function loadCheckMain(){	
	$.getJSON("/ccheckMainLcController/cCheckMainList.do",function(data){
		...
		$.each(data,function(i,v){
				...
				h+="<td role='button' tabindex='0'><input name='checkMainBox' type='checkbox'></td>";
