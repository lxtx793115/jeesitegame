...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/updownreport/upAndDownReport/listdown">下分管理</a></li>
		<li ><a href="${ctx}/updownreport/upAndDownReport/list">上下分报表</a></li>
		
	</ul>
	<form:form id="searchForm" modelAttribute="upAndDownReport" action="${ctx}/updownreport/upAndDownReport/listdown" method="post" class="breadcrumb form-search">
	...
	<table id="contentTabledown" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
		...
		<c:forEach items="${page.list}" var="upAndDownReport">
			<tr>
				...
				<shiro:hasPermission name="updownreport:upAndDownReport:edit"><td>
    				<a class="btn btn-primary" href="${ctx}/updownreport/upAndDownReport/Auditingsave?id=${upAndDownReport.id}&auditsta=1&shangxia=2" onclick="return confirmx('确认要同意改订单吗？', this.href)">同意请求</a>
    			<a class="btn btn-primary" href="${ctx}/updownreport/upAndDownReport/Auditingsave?id=${upAndDownReport.id}&auditsta=2&shangxia=2" onclick="return confirmx('确认要拒绝改订单吗？', this.href)">拒绝请求</a>	
    				<%-- <a class="btn btn-primary" href="${ctx}/updownreport/upAndDownReport/form?id=${upAndDownReport.id}">修改</a> --%>
					<%-- <a class="btn btn-primary" href="${ctx}/updownreport/upAndDownReport/delete?id=${upAndDownReport.id}&shangxia=2" onclick="return confirmx('确认要删除该上下分报表吗？', this.href)">删除</a> --%>
